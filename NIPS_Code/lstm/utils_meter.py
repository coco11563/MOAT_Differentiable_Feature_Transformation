...
class FSDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # if self.swap: a = np.random.randint(0, 5) b = np.random.randint(0, 5) encoder_input = encoder_input[:4 * a]
        # + encoder_input[4 * a + 2:4 * a + 4] + \ encoder_input[4 * a:4 * a + 2] + encoder_input[4 * (a + 1):20 + 4
        # * b] + \ encoder_input[20 + 4 * b + 2:20 + 4 * b + 4] + encoder_input[20 + 4 * b:20 + 4 * b + 2] + \
